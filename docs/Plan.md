...
- memory extraction 은 우선 `/voice/turn` 과 분리된 단계로 시작하고, 이후 자동화 여부를 검토한다.
- 중요한 정보 확인과 연속 인식 실패 복구는 optional polish 가 아니라 Voice Interview acceptance 에 포함한다.
...
- 중요한 정보는 짧은 확인 질문 또는 텍스트 확인 경로로 복구 가능하게 설계
...
- 연속 인식 실패 시 더 쉬운 재질문 또는 텍스트 확인 경로가 정의된다.
